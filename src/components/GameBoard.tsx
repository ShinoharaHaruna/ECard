...
export const GameBoard = () => {
    ...
    setTimeout(() => {
      setGameState((prev) => {
        ...
        if (!prev.playerSelection || !prev._pendingComputerMove) {
          console.warn('Missing player selection or computer move');
          return {
            ...prev,
  ...
  const nextRound = useCallback(() => {
    ...
    const { playerCards, computerCards, playerHasEmperor } = distributeCards(
      deck,
    ...
    <div className="container mx-auto p-4 max-w-4xl">
      ...
      <div className="space-y-8 bg-white p-6 rounded-lg shadow-lg">
        <div className="flex justify-between items-center">
          <div className="flex items-center space-x-2">
            <h2 className="text-xl font-semibold">{t.gameStatus.gameTitle}</h2>
          </div>
          ...
          <div className="flex items-center space-x-4">
            <div className="flex items-center">
              ...
              <span className="ml-1 font-bold">{formatMoney(gameState.playerMoney, language)}</span>
            </div>
            <div className="flex items-center">
              <span className="font-bold">{t.gameStatus.chips}: {gameState.playerChips}</span>
            </div>
          </div>
        </div>
        ...
        {gameState.gameStatus === 'betting' && (
          <div className="text-center space-y-4">
            <h3 className="text-xl font-semibold">{t.gameStatus.betting}</h3>
            ...
            <Button onClick={handleBet}>{t.gameStatus.betButton}</Button>
            {betError && (
              <p className="text-red-500">{betError}</p>
            )}
          </div>
        )}
        ...
        <div className="flex flex-col gap-8">
          <div className="space-y-4">
            <h3 className="text-lg font-medium text-center">{t.gameStatus.computerCards}</h3>
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-medium text-center">{t.gameStatus.yourCards}</h3>
            <div className="flex justify-center space-x-4">
          ...
          <div className="text-center space-y-4">
            <h3 className="text-2xl font-bold">
              {gameState.roundWinner === 'player'
                ? gameState.playerSelection?.type === 'Slave' && gameState.computerSelection?.type === 'Emperor'
                  ? `${t.gameStatus.slaveWin} ${formatMoney(gameState.currentBet * 500000, language)}!`
                  : `${t.gameStatus.normalWin} ${formatMoney(gameState.currentBet * 100000, language)}!`
        ...
        {gameState.gameStatus === 'gameOver' && (
          <div className="text-center space-y-4">
            ...
            <p className="text-xl">
              {t.gameStatus.finalResult}: {formatMoney(gameState.playerMoney, language)}，
              {t.gameStatus.remainingChips}: {gameState.playerChips}
            </p>
            <Button onClick={resetGame}>{t.gameStatus.playAgain}</Button>
          </div>
        )}
      </div>
      ...
      {DEBUG_MODE && (
        <div className="mt-8 p-4 border border-yellow-500 rounded-lg">
          <h3 className="text-lg font-bold text-yellow-500 mb-2">Debug Info</h3>
          <div className="space-y-2 text-sm">
            <p>Game Status: {gameState.gameStatus}</p>
            <p>Current Round: {gameState.currentRound}</p>
            <p>Current Bet: {gameState.currentBet}</p>
            <p>Computer Cards: {gameState.computerCards.map((card) => card.type).join(', ')}</p>
            <p>Computer Selection: {gameState.computerSelection?.type || 'None'}</p>
            <p>Player Cards: {gameState.playerCards.map((card) => card.type).join(', ')}</p>
            <p>Player Selection: {gameState.playerSelection?.type || 'None'}</p>
            <p>Money: {formatMoney(gameState.playerMoney, language)}</p>
