...
export function saveGameState(playerHadEmperor: boolean): void {
    localStorage.setItem(
        "lastPlayerHadEmperor",
        JSON.stringify(playerHadEmperor)
    );
}
...
export function distributeCards(
    deck: Card[],
    lastPlayerHadEmperor: boolean | null
): { playerCards: Card[]; computerCards: Card[]; playerHasEmperor: boolean } {
    // 首先找到皇帝和奴隶的位置
    ...
    // 否则根据上一局的状态交替分配
    const isEmperorForPlayer =
        lastPlayerHadEmperor === null ? true : !lastPlayerHadEmperor;
